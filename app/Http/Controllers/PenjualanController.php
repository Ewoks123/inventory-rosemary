...
class PenjualanController extends Controller
{
    private function getSortedProduks()
    {
        ...
        return Produk::all()->sort(function($a, $b) use ($masterProduks) {
    ...
    public function index()
    {
        ...
        return view('produk.penjualan', compact('produks', 'sales'));
    }

    public function create()
    {
        $produks = $this->getSortedProduks();
        return view('produk.penjualan', compact('produks'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($produk->stok_produk < $request->jumlah_terjual) {
            return back()->withErrors(['jumlah_terjual' => 'Stok tidak mencukupi.'])->withInput();
        }

        $produk->stok_produk -= $request->jumlah_terjual;
        $produk->save();
        ...
        return redirect()->route('produks.index')->with('success', 'Penjualan berhasil disimpan.');
    }

    public function show($id)
    {
        $sale = Penjualan::with('produk')->findOrFail($id);
        return view('produk.sale-detail', compact('sale'));
    }
    ...
    public function edit($id)
    {
        ...
        $produks = $this->getSortedProduks();
        return view('produk.penjualan-edit', compact('sale', 'produks'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $oldQuantity = $sale->jumlah_terjual;
        $produk = Produk::findOrFail($request->id_produk);

        if ($sale->id_produk !== $request->id_produk) {
            $oldProduct = Produk::findOrFail($sale->id_produk);
            $oldProduct->stok_produk += $oldQuantity;
            $oldProduct->save();
        }

        if ($produk->stok_produk + ($sale->id_produk !== $request->id_produk ? 0 : $oldQuantity) < $request->jumlah_terjual) {
            return back()->withErrors(['jumlah_terjual' => 'Stok tidak mencukupi untuk pembaruan.'])->withInput();
        }

        $produk->stok_produk = $produk->stok_produk + ($sale->id_produk !== $request->id_produk ? 0 : $oldQuantity) - $request->jumlah_terjual;
        $produk->save();
    ...
    {
        $sale = Penjualan::findOrFail($id);
        $produk = Produk::findOrFail($sale->id_produk);
        $produk->stok_produk += $sale->jumlah_terjual;
        $produk->save();
        $sale->delete();
    ...
    public function report(Request $request)
    {
        ...
        $reportRoute = 'penjualans.report';
        $reportPdfRoute = 'penjualans.report.pdf';
        $reportExcelRoute = 'penjualans.report.excel';
        $reportBackRoute = 'penjualans.index';
    ...
    public function exportExcel(Request $request)
    {
        $startDate = $request->input('start_date', date('Y-m-01'));
        $endDate = $request->input('end_date', date('Y-m-t'));
        return Excel::download(new ProdukExport($startDate, $endDate), 'Laporan_Penjualan_'.$startDate.'_to_'.$endDate.'.xlsx');
    }
